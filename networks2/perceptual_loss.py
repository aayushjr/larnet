...
class InceptionI3d_perceptual(nn.Module):
    ...
    def __init__(self, name='modified_inception_i3d', final_endpoint='Mixed_4b', in_frames=16, in_channels=3,
                 pretrained=True, weights_path='/home/namanb/Desktop/UCF_work/models/rgb_charades.pt', weights=[]):
        """
        ...
        super(InceptionI3d_perceptual, self).__init__()
        self.i3d_name = name
        ...
        self.feature_layer = Unit3D(in_channels=384 + 384 + 128 + 128, output_channels=256, kernel_shape=[3, 3, 3],
                                    padding=1,activation_fn=torch.tanh, name='features')  # 256 + 320 + 128 + 128
    ...
    def forward(self, x1,x2):
        ...
        for end_point in self.VALID_ENDPOINTS:
            if end_point in self.end_points:
                x1 = self._modules[end_point](x1)
                x2 = self._modules[end_point](x2)
                # print(x.size())
                # use _modules to work with dataparallel
                if end_point in ['Mixed_4b','Mixed_3c','Conv3d_1a_7x7','Conv3d_2b_1x1']:
                    features_pred.append(x1)
                    features_real.append(x2)
#                     print(x1.size())
        loss = self.weights[0] * self.mse_loss(features_pred[0], features_real[0])
        loss += self.weights[1] * self.mse_loss(features_pred[1], features_real[1])
        loss += self.weights[2] * self.mse_loss(features_pred[2], features_real[2])
        loss += self.weights[3] * self.mse_loss(features_pred[3], features_real[3])

        return loss  # returns 64x8x56x56, 192x8x28x28, 256x4x14x14
        # return x


def load_layer_weights(weights_path='/home/namanb/Desktop/UCF_work/models/rgb_charades.pt'):
    state_dict = torch.load(weights_path)
    remove_layers = ['Logits', 'Predictions','Mixed_5a','Mixed_5b','Mixed_4c','Mixed_4d','Mixed_5c','Mixed_4e','Mixed_4f']
...
def get_patch(vid):
    bsz, c, t, h, w = vid.size()
    t1 = random.randint(0, t-4)
    t2 = t1+4
    h1 = random.randint(0, h-14)
    w1 = random.randint(0, w-14)
    
    patch = torch.stack([vid[:,:,t1+i,h1:h1+14,w1:w1+14] for i in range(4)], 2 )
    return patch

if __name__ == "__main__":
    print_summary = True

    i3d = InceptionI3d(final_endpoint='Mixed_4b', in_frames=16)
    x1 = torch.randn(1,3,16,112,112)
    x2 = torch.randn(1,3,16,112,112)
    y = i3d(x1,x2)
    print(y)
